#include "Player.hh"
#include <queue>
#define PLAYER_NAME Zarathustra8
...
struct PLAYER_NAME : public Player
{
    ...
    void BFS_food(int &dist, Dir &opt_dir, Pos p1)
    {
        ...
        {
            Dir d = dirs[i];
            if (pos_correct(p1 + d) and not thereis_dead(p1 + d, 1))
            {
                Q.push({d,
                        p1 + d,
                        1});
    ...
    movement best_dir(Pos p)
    {
        ...
        else
        {
            ...
            else if (dist_food < dist_enemy) // and dist_food <= 7)
            {
